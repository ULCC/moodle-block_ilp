<?php
/**
* Allows the user to view a list of students entries for a partucular report
*
...
//get the state id chosen if set
...
if (empty($access_viewotherilp)  && !empty($course_id))	{
    //the user doesnt have the capability to view the ilps of others
...
if (!empty($course_id)) {
    $listurl	=	"{$CFG->wwwroot}/blocks/ilp/actions/view_studentlist.php?tutor=0&course_id={$course_id}&report_id={$report_id}";
} else {
    $listurl	=	"{$CFG->wwwroot}/blocks/ilp/actions/view_studentlist.php?tutor=1&course_id=0&report_id={$report_id}";
}
...
if (!empty($course_id)) {
    ...
    $title		=	$course->shortname." ".get_string('reportentries','block_ilp');
} else {
    $title		=	get_string('mytutees','block_ilp')." ".get_string('reportentries','block_ilp');
}
...
if (!empty($course_id)) {
    ...
    $temp   =   new stdClass();
    $temp->report   =   $report->name;
    $pagetitle	=	get_string('tuteesentries','block_ilp',$temp);
}

$status_items	=	$dbc->get_status_items(ILP_DEFAULT_USERSTATUS_RECORD);
$baseurl		=	$CFG->wwwroot."/blocks/ilp/actions/view_studentreports.php";
//require the view_studentlist.html page

require_once($CFG->dirroot.'/blocks/ilp/views/view_studentreports.html');
