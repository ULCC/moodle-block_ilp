...
class add_graph_mform extends ilp_moodleform {
		public		$report_id;
		public		$dbc;
		...
		function __construct($report_id) {
			...
			// call the parent constructor
       	 	parent::__construct("{$CFG->wwwroot}/blocks/ilp/actions/edit_report_graphs.php?report_id={$this->report_id}");
		}
		...
		function definition() {
			...
			$formelementplugins		=	$dbc->get_graph_plugins();
        	...
        	$mform->addElement('select', 'plugin_id', get_string('addgraph', 'block_ilp'), $frmplugins);
        	$mform->addRule('plugin_id', null, 'required', null, 'client');
	        $mform->setType('plugin_id', PARAM_INT);
	        
	        $buttonarray[] = &$mform->createElement('submit', 'submitbutton', get_string('addgraph','block_ilp'));
