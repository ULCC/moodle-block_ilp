...
class ilp_dashboard_mis_misc_tab  extends ilp_dashboard_tab {
	...
	function display_name()	{
		return	get_string('ilp_dashboard_mis_misc_tab_name','block_ilp');
	}
	...
	function display($selectedtab=null)	{
		...
		if ($this->dbc->get_user_by_id($this->student_id)) {
				...
				if ((isset($seltab[1]) && !empty($seltab[1])) || !empty($this->secondrow))  {
					...
					require_once $CFG->dirroot.'/blocks/ilp/classes/dashboard/mis/'.$misplugin->name.'.php';
	 ...
	 */
	 function language_strings(&$string) {
        $string['ilp_dashboard_mis_misc_tab'] 					= 'MISC MIS PLUGINS';
        $string['ilp_dashboard_mis_misc_tab_name'] 				= 'MIS Plugins';
