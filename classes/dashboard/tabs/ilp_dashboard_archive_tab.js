...
function toggle_container(elem, from, to) {
	...
	if(to == 0) {
		...
		// add the closed icon
		document.getElementById(elem.id+'_icon').setAttribute('src', M.ilp_dashboard_archive_tab.closed_image);
		...
		// add the open icon
		document.getElementById(elem.id+'_icon').setAttribute('src', M.ilp_dashboard_archive_tab.open_image);
	}
