...
M.ilp_dashboard_reports_tab = {
    ...
    toggle_container: function(elem, from, to) {
        // remove all click events previously attached to this element
        elem.detach('click');
        ...
        if(to == 0) {
            ...
            page.setStyle("height",M.ilp_dashboard_reports_tab.get_height(page)+"px");

            // reset the desired height in case ajax has expanded the content
            from = M.ilp_dashboard_reports_tab.get_height(container);

            // add the closed icon
            Y.one('#'+element_id+'_icon').set('src', M.ilp_dashboard_reports_tab.closed_image);
            ...
            // add the open icon
            Y.one('#'+element_id+'_icon').set('src', M.ilp_dashboard_reports_tab.open_image);
        ...
        animation.on('end', function(e){
            elem.on('click',function () { M.ilp_dashboard_reports_tab.toggle_container(elem, to, from);});
