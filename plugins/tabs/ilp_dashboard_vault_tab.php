...
class ilp_dashboard_vault_tab extends ilp_dashboard_tab {
    ...
    public  function reportsoverview()  {
        ...
        $reports		=	$this->dbc->get_reports_vaulted(null,null,false);
        $reportslist	=	array();
        if (!empty($reports)) {
                ...
                if (!empty($reportentries)) {
                    ...
                    foreach ($reportentries as $entry)	{
                        ...
                        if (!empty($reportfields)) {
                            ...
                            $comments      = $this->dbc->get_entry_comments($entry->id);
                            if($comments){
                                $report_entry .= '<div id="'. $entry->id .'" class="comment_counter">Comments found: '. count($comments) .'</div>';
    ...
    function print_entries($reportentries, $dbc){
        global $CFG;
        $reportentry    =  "";
