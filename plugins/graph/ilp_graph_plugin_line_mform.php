...
class ilp_graph_plugin_line_mform extends ilp_graph_plugin_mform {
    ...
    protected function specific_definition($mform) {
        ...
        $optionlist[0]  =   get_string('average','block_ilp');
        $optionlist[1]  =   get_string('total','block_ilp');

         $mform->addElement(
                  'select',
                  'calculation',
                  get_string( 'ilp_graph_plugin_line_calculation' , 'block_ilp' ),
                  $optionlist
         );
            ...
            foreach ($reportfields as $rf)  {
             ...
             $repeatelements     =        array(
                 $mform->createElement(
                     'select',
                     'reportfield_id',
                     get_string( 'ilp_graph_plugin_line_reportfield' , 'block_ilp' ),
                     $optionlist
                 )
            );
    ...
    protected function specific_process_data($data) {
        ...
        $plgrec = (!empty($data->reportgraph_id)) ? $this->dbc->get_graph_by_report("block_ilp_plu_graph_line",$data->reportgraph_id) : false;

        if (!empty($plgrec)) {
            //delete all records already attached to this graph as we will be recreating them all
            $this->dbc->delete_record("block_ilp_plu_graph_line",array('reportgraph_id'=>$data->reportgraph_id));

        }

        $fieldcount  =   count($data->reportfield_id);

        for($i=0; $i < $fieldcount; $i++)   {
            $itemrecord = new stdClass();
            $itemrecord->reportfield_id  =   $data->reportfield_id[$i];
            $itemrecord->reportgraph_id  =   $data->reportgraph_id;
            $itemrecord->calculation     =   $data->calculation;

            $this->dbc->create_plugin_record("block_ilp_plu_graph_line",$itemrecord);
        }
    }
