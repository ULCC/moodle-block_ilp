...
/**
    ...
    function get_coursereports($course_id,$report_id=null,$status=null) {
    	...
    	$sql	=	"SELECT		*, cr.id as cr_id
    				 ...
    				 AND		course_id 	=  :course_id	/*{$course_id}*/
    				{$report_sql}
    				{$status_sql}";
...
/**
    ...
    function get_pluginentry($tablename,$entry_id,$reportfield_id,$multiple=false) {
		...
		$sql	=	"SELECT		*
					 FROM 		{$parenttable} as p,
					 			...
					 			{$entrytable} as e
					 WHERE 		{$where}
					 AND		e.entry_id	=	:entry_id /*{$entry_id}*/
					 AND		p.reportfield_id	=	:reportfield_id /*{$reportfield_id}*/
					 ";
	...
	function get_entrystatus($entry_id,$reportfield_id)	{
		...
		$sql	=	"SELECT			*
					 ...
					 AND			se.entry_id			=	:entry_id /*{$entry_id}*/
					 AND			s.reportfield_id	=	:reportfield_id /*{$reportfield_id}*/";
    ...
    */
    function plugin_data_item_exists( $tablename, $reportfield_id, $item_table=false, $item_key=false, $item_value_field=false ){
		...
		$sql = "SELECT *
				...
				WHERE ele.reportfield_id = :reportfield_id /*{$reportfield_id}*/
				";
    ...
    */
    ...
    function get_status_options( $reportfield_id )	{
        ...
        $params = array('reportfield_id'=>$reportfield_id);
    		...
    		$sql	=	"SELECT i.id, i.name, i.value
    					 ...
    					 AND	rs.reportfield_id	=	:reportfield_id /*{$reportfield_id}*/
    					 ";
    ...
    function get_user_report_entries($report_id,$user_id,$state_id=null)	{
        ...
        if (!empty($state_id)) {
    		...
    		$where		=	" 	AND e.id	=	se.entry_id
    							AND	se.parent_id	=	:state_id"; // {$state_id}";
    	}
    ...
    function get_report_stateitems($report_id)	{
    	...
    	$sql	=	"SELECT		*
    	 			...
    	 			AND			rf.report_id	=	:report_id /*{$report_id}*/
    ...
    */
    public function element_type_exists( $report_id , $tablename ){
        ...
        $sql = "
            ...
            WHERE rpt.id = $report_id AND pln.tablename = :tablename /*'$tablename'*/
        ";
        $res = $this->dbc->get_record_sql( $sql, $params );
        return $res->n;
    }
    ...
    public	function get_template_plugins($templatename)	{
        ...
        $params = array('templatename'=>$templatename);
    	...
    	$sql	=	"SELECT 	tr.name as region_name,
    				 ...
    				 AND 		t.name	=   :templatename	/*'{$templatename}'*/
    				 ";
    ...
    */
    public function get_next_position( $report_id, $tablename ){
        ...
        $tablename = $CFG->prefix . $tablename;
        $sql = "SELECT MAX( position ) n FROM  $tablename WHERE report_id = :report_id /*$report_id*/";  //gregp - records should be counted not the position #
        $res = array_values( $this->dbc->get_records_sql( $sql, $params ) );
        $n = $res[0]->n;
        if( $n ){
            return $n + 1;
        }
    ...
    public function count_report_entries($report_id,$user_id)	{
    	...
    	$sql	=	"SELECT		*
    				 ...
    				 AND		report_id		=	:report_id	/*{$report_id}*/";
    ...
    public function	has_plugin_field($report_id,$pluginname)	{
        ...
        $params = array('report_id'=>$report_id, 'pluginname'=>$pluginname);
    	...
    	$sql	=	"SELECT			*
    				 ...
    				 WHERE			rf.plugin_id	=	p.id
    				 AND			rf.report_id	=	:report_id /*{$report_id}*/
    				 AND			p.name			=   :pluginname /*'{$pluginname}'*/
    				 ";
        return 		$this->dbc->get_records_sql($sql, $params);
    }
...
/**
    ...
    public	function count_report_entries_with_state($report_id,$user_id,$state,$count=true,$entry_id=false)	{
    		...
    		$sql	=	"SELECT		{$select}
    					 ...
    					 AND		e.report_id		=	:report_id /*{$report_id}*/
    					 AND		e.user_id		=	:user_id /*{$user_id}*/
    					 AND		pi.passfail		=	:state /*{$state}*/
    					 $entrysql";
    ...
    public	function get_reports_in_period($ltimestamp,$utimestamp)	{
        ...
        $params = array('ltimestamp'=>$ltimestamp, 'utimestamp'=>$utimestamp);
    	...
    	$sql	=	"SELECT 		e.*, r.*,ddl.value as deadline
					   FROM			{block_ilp_entry} 			AS e,
						...
						WHERE		stitems.passfail = 0
						  AND		ddl.value >= :ltimestamp /*{$ltimestamp}*/
						  AND		ddl.value < :utimestamp /*{utimestamp}*/
        ...
        $sql    =   "SELECT MAX(timemodified) AS timemodified
                     FROM (
                            (
                                ...
                                FROM	{block_ilp_entry}
                                WHERE	report_id	=	:report_id1 /*{$report_id}*/
                                AND 	user_id		=	:user_id1 /*{$user_id}*/
                            )
                            UNION
                            (
                                ...
                                WHERE 	entry_id = e.id
                                AND	report_id	=	:report_id2 /*{$report_id}*/
                                AND	user_id		=	:user_id2 /*{$user_id}*/
                            )
                            {$statussql}
			            ) AS lastreportupdate
                     ";
...
";
    ...
    */
  	function has_statusfield($status_id,$report_id)	{
        ...
        $params = array('status_id'=>$status_id, 'report_id'=>$report_id);
  		...
  		return ($this->dbc->get_records_sql($sql, $params)) ? true: false;
  	}
            ...
            if(!empty($group_id)){
	 		...
	 		$sql	=	"SELECT		distinct(u.id)
	 					  FROM		{user} u INNER JOIN {role_assignments} ra on u.id = ra.userid
	 					  			{$grouptable}
	 					  			LEFT OUTER JOIN {user_lastaccess} ul on (ra.userid = ul.userid and ul.courseid = :course_id1 /*{$course_id}*/)
	 					  			LEFT OUTER JOIN {role} r on ra.roleid = r.id

	 					  WHERE		(ra.contextid = {$context->id} OR ra.contextid in {$listofcontexts})
	 					    AND		u.deleted = 0
	 					    AND		(ul.courseid = :course_id2 /*{$course_id}*/ OR ul.courseid IS NULL)
    	...
    	$sql	=	"SELECT MAX(timemodified) AS timemodified
                     FROM (
                            (
                                SELECT MAX(timemodified) AS timemodified
                                FROM	{block_ilp_entry}
                                WHERE	user_id		=   :user_id1	/*{$user_id}*/
                            )
                            UNION
                            (
                                SELECT 	MAX(ec.timemodified) AS timemodified
							...
							UNION
                             (
                                SELECT	timemodified
                                 FROM	{block_ilp_user_status}
                                 WHERE	user_id		=	:user_id3	/*{$user_id}*/
                              )
...
";
    ...
    function get_report_coursefield($entry_id,$reportfield_id)	{
        ...
        $params = array('reportfield_id'=>$reportfield_id, 'entry_id'=>$entry_id);

    	$sql	=	"SELECT 	*
    				 FROM		{block_ilp_plu_crs} as c,
    				 			{block_ilp_plu_crs_ent} as ce
    				 WHERE		c.id = ce.parent_id
    				 AND		c.reportfield_id 	= 	:reportfield_id /*{$reportfield_id}*/
    				 AND		entry_id			=	:entry_id /*{$entry_id}*/";
    ...
    function get_calendar_event($entry_id,$reportfield_id)	{
        ...
        $sql	=	"SELECT		e.*
    				 ...
    				 AND		ce.reportfield_id	=	:reportfield_id /*{$reportfield_id}*/
    				 AND		ce.entry_id			=	:entry_id /*{$entry_id}*/";
